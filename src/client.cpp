 ...
 */
void RunClient(const std::string& host, int port, std::string username) {
  ...
  int client_socket = socket(address_info->ai_family, address_info->ai_socktype,
  ...
  if (connect(client_socket, address_info->ai_addr, address_info->ai_addrlen) <
      0) {
    freeaddrinfo(address_info);
    close(client_socket);
  ...
  Connection conn(client_socket, false, username);
  conn.Run();
}