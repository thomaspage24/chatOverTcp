...
std::vector<uint8_t> encrypt(const std::string& plaintext, const uint8_t key[AES_KEY_SIZE]) {
    ...
    if (EVP_EncryptInit_ex(cipher_ctx, EVP_aes_256_cbc(), nullptr, key, iv) != 1) {
...
std::string decrypt(const std::vector<uint8_t>& iv_and_ciphertext,
    ...
    if (EVP_DecryptInit_ex(cipher_ctx, EVP_aes_256_cbc(), nullptr, key, iv) != 1) {
        EVP_CIPHER_CTX_free(cipher_ctx);
    ...
    if (EVP_DecryptFinal_ex(cipher_ctx,
                            plaintext.data() + total_bytes,
                            &chunk_size) != 1) {
        EVP_CIPHER_CTX_free(cipher_ctx);
