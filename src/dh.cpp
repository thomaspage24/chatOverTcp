...
static bool raw_send(int socket_fd, const uint8_t* data, size_t num_bytes) {
    ...
    while (bytes_sent < num_bytes) {
        ssize_t result = write(socket_fd, data + bytes_sent, num_bytes - bytes_sent);
...
void dh_handshake(int socket_fd, bool is_server, uint8_t out_key[AES_KEY_SIZE]) {
    ...
    if (EVP_PKEY_get_raw_public_key(our_key, our_public_bytes.data(), &pub_len) != 1) {
        EVP_PKEY_free(our_key);
        throw std::runtime_error("failed to get raw public key");
    }

    // --- step 3: exchange public keys ---
    // server sends first, client receives first — both sides must agree on this order
    ...
    EVP_PKEY* their_key = EVP_PKEY_new_raw_public_key(
        ...
        their_public_bytes.data(), their_public_bytes.size());
    if (!their_key) {
        EVP_PKEY_free(our_key);
