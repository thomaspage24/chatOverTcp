...
void RunServer(int port, std::string username) {
  ...
  if (listening_socket < 0) throw std::runtime_error("socket() failed");
  ...
  int client_socket =
      accept(listening_socket, (sockaddr*)&client_address, &client_address_len);
  ...
  Connection conn(client_socket, true, username);
  conn.Run();
}
